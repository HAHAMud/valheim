...
import { getObtainMethods } from '@/module/item';
import { FoodType } from '@/module/item/types';

const Card = ({ item }: { item: FoodType }) => {
  return (
    <div
    ...
    >
      ...
      <div className="relative mt-6 px-6 pb-6 text-white">
        <span className="-mb-1 block opacity-75">{item.maxCount}</span>
        <div className="flex justify-between">
          ...
          {item.obtainMethods && (
            ...
            >
              {getObtainMethods(item.obtainMethods)}
            </span>
          )}
        </div>
      </div>
...
export const ItemCard = ({ list }: { list: FoodType[] }) => {
  return (
